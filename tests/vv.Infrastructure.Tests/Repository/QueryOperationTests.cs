...
namespace vv.Infrastructure.Tests.Repository
{
    public class QueryOperationTests : BaseMarketDataRepositoryTests
    {
        ...
        public async Task QueryAsync_ShouldReturnFilteredResults()
        {
            ...
            // Filter the data to match the expected query
            var filteredData = data.Where(e => e.AssetId == "eurusd").ToList();
            ...
            // Use the filtered data for the mock response
            var mockResponse = new Mock<FeedResponse<FxSpotPriceData>>();
            mockResponse.Setup(r => r.GetEnumerator()).Returns(filteredData.GetEnumerator());
            ...
            // This is the key fix - return the filtered data instead of all data
            mockQueryProvider
            ...
            mockQueryable.As<IQueryable<FxSpotPriceData>>()
                ...
                .Returns(filteredData.AsQueryable().Expression);

            mockQueryable.As<IQueryable<FxSpotPriceData>>()
                .Setup(q => q.ElementType)
                .Returns(filteredData.AsQueryable().ElementType);

            mockQueryable.As<IQueryable<FxSpotPriceData>>()
                .Setup(q => q.GetEnumerator())
                .Returns(filteredData.GetEnumerator());
